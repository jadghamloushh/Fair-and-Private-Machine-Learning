...
# Load data
def load_loan_data(train_file: str, test_file: str, batch_size: int = 512):
...
def train_model(model, train_loader, test_loader, optimizer, scheduler, epochs: int = 50,
        ...
        for batch_idx, (data, target) in enumerate(train_loader):
            ...
            if (batch_idx + 1) % 20 == 0:
                ...
                print(f'Epoch: {epoch+1}, Batch: {batch_idx+1}, '
                      f'Avg Loss: {current_batch_loss:.3f}, '
                      f'Batch Train Acc: {current_train_acc:.2f}%')
    ...
    return all_train_accs, all_test_accs, all_train_losses
...
def evaluate_equal_opportunity(model, test_loader, sensitive_values):
    ...
    # Compute TPR for each sensitive group
...
# Plotting Function
def plot_results(train_accs, test_accs, train_losses, epochs):
...
def main():
    ...
    for run_idx, run_seed in enumerate(run_seeds):
        ...
        try:
            ...
            train_loader, test_loader, train_size, input_dim = load_loan_data("train.csv", "test.csv", batch_size=BATCH_SIZE)
    ...
    plot_results(avg_train_accs, avg_test_accs, avg_train_losses, EPOCHS)
