...
def load_adult_data(file_path):
    ...
    # Convert income to binary target ('>50K' = 1, '<=50K' = 0)
    data['income'] = data['income'].apply(lambda x: 1 if x == '>50K' else 0)
    
    # Map gender to binary values (Male = 1, Female = 0)
    data['gender'] = data['gender'].apply(lambda x: 1 if x == 'Male' else 0)
    
    # Extract target variable (income) and sensitive attribute (gender)
...
X_full, y_full, sensitive_full = load_adult_data('adult.csv')

# Split into train, validation, and test sets (70% train, 10% validation, 20% test)
X_train_val, X_test, y_train_val, y_test, sensitive_train_val, sensitive_test = train_test_split(
    X_full, y_full, sensitive_full, test_size=0.2, random_state=42
)

X_train, X_val, y_train, y_val, sensitive_train, sensitive_val = train_test_split(
    X_train_val, y_train_val, sensitive_train_val, test_size=0.125, random_state=42  # 0.125 * 0.8 = 0.1 (10% of full dataset)
)

# Identify categorical and numerical columns
# Adjust these based on your adult dataset columns
categorical_cols = ['workclass', 'education', 'marital-status', 'occupation', 
                   'relationship', 'race', 'native-country']
numerical_cols = ['age', 'fnlwgt', 'education-num', 'capital-gain', 
                 'capital-loss', 'hours-per-week']

# Remove 'gender' from feature lists if it's included
...
l2_norm_clip = 1.0
batch_size = 240  # Adjusted to be divisible by more values
epochs_per_iteration = 60
# Set microbatches to be a divisor of batch_size to avoid the "requires a multiple of" error
microbatches = 8  # This divides 240 evenly
learning_rate = 0.15

# Print dataset sizes to help debug issues
    ...
    def fit(self, X, y, sample_weight=None):
        if sample_weight is not None:
            ...
            if np.isnan(sample_weight).any():
                print("Warning: NaN values in sample weights. Replacing with 1.0")
                sample_weight = np.where(np.isnan(sample_weight), 1.0, sample_weight)
...
egr = ExponentiatedGradient(
    ...
    eps=0.01,  
    max_iter=5,  # Reduced from 8 to 5 to make training faster
    nu=1e-6  
)
...
def plot_group_roc_curves(y_true, y_score, sensitive_features, title="ROC Curves by Group"):
    ...
    plt.grid(True)
    plt.savefig('adult_group_roc_curves.png')
    plt.close()
...
def plot_tpr_at_thresholds(y_true, y_score, sensitive_features, thresholds=None, title="TPR by Group at Different Thresholds"):
    ...
    plt.title(title)
    plt.legend(loc='best')
    plt.savefig('adult_tpr_vs_threshold.png')
    plt.close()
...
def plot_tpr_gap(male_tpr, female_tpr, thresholds, title="TPR Parity Gap at Different Thresholds"):
    ...
    plt.title(title)
    plt.savefig('adult_tpr_gap_vs_threshold.png')
    plt.close()
...
def plot_fairness_metrics_comparison(metrics):
    ...
    fig.tight_layout()
    plt.savefig('adult_fairness_metrics_comparison.png')
    plt.close()
...
if len(np.unique(y_proba)) > 2:  # Only plot if we have more than binary values
    auc_metrics = plot_group_roc_curves(y_test, y_proba, sensitive_test,
                                        title="ROC Curves by Gender Group (Adult Dataset)")
    ...
    male_tpr, female_tpr, thresholds = plot_tpr_at_thresholds(
        ...
        title="True Positive Rate by Gender at Different Thresholds (Adult Dataset)"
    )
   
    # Plot TPR gap at different thresholds
    min_gap, min_gap_threshold = plot_tpr_gap(
        male_tpr, female_tpr, thresholds,
        title="TPR Parity Gap at Different Thresholds (Adult Dataset)"
    )
...
print("\nPlots saved:")
print("1. adult_group_roc_curves.png - ROC curves comparing model performance between gender groups")
print("2. adult_tpr_vs_threshold.png - How TPR varies by gender across different classification thresholds")
print("3. adult_tpr_gap_vs_threshold.png - The TPR gap between gender groups at different thresholds")
print("4. adult_fairness_metrics_comparison.png - Bar chart comparing key fairness metrics by gender")

if __name__ == "__main__":
    print("Running DP-Fair model on Adult dataset")
    # The main execution is already handled in the script above